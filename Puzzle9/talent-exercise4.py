...
def Hire4Show(candList, candTalents, talentList):
    ...
    n = len(candList)
    hire = candList[:]
    guarantee_1 = sum([i[1] for i in d_candList])
    guarantee_2 = sum([i[1] for i in candList])
    for i in range(2**n):
        Combination = []
        num = i
        for j in range(n): 
        ...
        if Good(Combination, candList, candTalents, talentList):
            if guarantee_2 > Combination_g:
                hire = Combination
                guarantee_2 = Combination_g
    #並び替え
    final_dicision = []
    for i in candidates:
    ...
    print ('Optimum Solution:', final_dicision)
    print ('weight is:', guarantee_1 + guarantee_2)
