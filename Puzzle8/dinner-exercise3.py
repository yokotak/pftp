...
#一組を除いて嫌いなペアを取り除いたリストの作成
def removeBadCombinations_oneremain(allCombL, dislikePairs):
    allGoodCombinations = []
    for i in allCombL:
        good = 0
        for j in dislikePairs:
...
def InviteDinnerFlexible(guestList, dislikePairs):
    allCombL = Combinations(len(guestList), guestList)
    allGoodCombinations = removeBadCombinations_oneremain(allCombL, dislikePairs)
...
InviteDinnerFlexible(guestList, dislikePairs)
